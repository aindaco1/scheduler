...
function renderTypes() {
  $("#panel-types", app).innerHTML =
    `<div class="admin-layout"><section class="panel"><div class="between"><h2>${t("Meeting types", "Tipos de reunión")}</h2><button class="button" type="button" data-add-type>+ ${t("New type", "Nuevo tipo")}</button></div><p>${t("Keep each invitation clear: a duration, a purpose, and a place to connect.", "Cada invitación debe ser clara: una duración, un propósito y un lugar para conectar.")}</p>${settings.types.map((type, i) => `<details class="editor-card" ${i === 0 ? "open" : ""}><summary>${esc(local(type.name) || t("New meeting", "Nueva reunión"))} · ${type.duration} min</summary><div class="stack">${bilingual(`types.${i}.name`, t("Name", "Nombre"))}${bilingual(`types.${i}.description`, t("Description", "Descripción"), true)}<div class="form-grid">${field(`types.${i}.duration`, t("Duration (minutes)", "Duración (minutos)"), { type: "number", min: 5, max: 240 })}<label class="field">${t("Meeting service", "Modalidad de reunión")}<select data-setting="types.${i}.mode"><option value="meet" ${type.mode === "meet" ? "selected" : ""}>Google Meet</option><option value="zoom" ${type.mode === "zoom" ? "selected" : ""}>Zoom</option><option value="in-person" ${type.mode === "in-person" ? "selected" : ""}>${t("In person", "Presencial")}</option></select></label></div><div data-gap-editor="${i}">${gapEditor(i)}</div><fieldset><legend>${t("Allowed in-person locations", "Lugares presenciales permitidos")}</legend>${settings.locations.length ? settings.locations.map((l) => `<label class="check"><input type="checkbox" data-type-location="${i}" value="${esc(l.id)}" ${type.locationIds.includes(l.id) ? "checked" : ""}>${esc(local(l.name))}</label>`).join("") : `<p class="help-text">${t("Add locations below to offer in-person meetings.", "Añade lugares a continuación para ofrecer reuniones presenciales.")}</p>`}</fieldset>${check(`types.${i}.enabled`, t("Active", "Activo"))}<div class="between"><small>${t("Link ID", "ID del enlace")}: ${esc(type.id)}</small><button class="button danger" type="button" data-remove="types" data-index="${i}" ${settings.types.length === 1 ? "disabled" : ""}>${t("Remove type", "Eliminar tipo")}</button></div></div></details>`).join("")}</section><section class="panel"><div class="between"><h2>${t("In-person locations", "Lugares presenciales")}</h2><button class="button" type="button" data-add-location>+ ${t("New location", "Nuevo lugar")}</button></div><p>${t("Each location has its own hours, inside your overall working hours. All hours use your schedule time zone.", "Cada lugar tiene su propio horario, dentro de tu horario general. Todos usan la zona horaria de tu agenda.")}</p>${settings.locations.map((l, i) => `<details class="editor-card" open><summary>${esc(local(l.name) || t("New location", "Nuevo lugar"))}</summary><div class="stack">${bilingual(`locations.${i}.name`, t("Location name", "Nombre del lugar"))}${bilingual(`locations.${i}.address`, t("Full street address", "Dirección postal completa"))}<p class="help-text">${t("Include street, city, state or region, postal code and country. This goes in the calendar invitation’s Location field.", "Incluye calle, ciudad, estado o región, código postal y país. Se incluirá en el campo Ubicación de la invitación.")}</p>${bilingual(`locations.${i}.instructions`, t("Arrival instructions (optional)", "Instrucciones de llegada (opcionales)"), true)}<p class="help-text">${t("Entry, parking or other details for your guest. Included in the invitation and booking emails; not shown on the public booking page.", "Acceso, estacionamiento u otros detalles para tu invitado. Se incluyen en la invitación y los correos de reserva; no aparecen en la página pública.")}</p>${hours(`locations.${i}.hours`, l.hours, t("Location hours", "Horario del lugar"))}${check(`locations.${i}.enabled`, t("Offer this location", "Ofrecer este lugar"))}<button class="button danger" type="button" data-remove="locations" data-index="${i}">${t("Remove location", "Eliminar lugar")}</button></div></details>`).join("")}</section></div>`;
