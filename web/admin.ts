...
function onSetting(event: Event) {
  ...
  if (el.dataset.defaultGap !== undefined) {
    const index = Number(el.dataset.defaultGap),
      type = settings.types[index];
    type.gap = (el as HTMLInputElement).checked
      ? null
      : defaultMeetingGap(settings, type.mode);
    syncGapEditors();
    $<HTMLInputElement>(`[data-default-gap="${index}"]`, app).focus();
    dirty();
    return;
  }
...
function gapEditor(index: number) {
  const type = settings.types[index],
    inherited = type.gap === null;
  const gap = defaultMeetingGap(settings, type.mode);
  return `<div class="form-grid gap-editor"><label class="check"><input type="checkbox" data-default-gap="${index}" ${inherited ? "checked" : ""}><span>${t("Use default gap", "Usar intervalo predeterminado")} · ${gap} min</span></label><label class="field">${t("Gap (minutes)", "Intervalo (minutos)")}<input type="number" min="0" max="240" step="1" required data-setting="types.${index}.gap" value="${type.gap ?? gap}" ${inherited ? "disabled" : ""}></label></div>`;
}
