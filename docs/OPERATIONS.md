...
4. For Zoom, create an OAuth app with the current scopes covering the authenticated user's details and meeting list/create/update/delete. Register `https://scheduler.dustwave.xyz/api/admin/callback/zoom`, save the client secrets, then connect from the dashboard. Keep Zoom types disabled until this passes. Meet needs no separate Google Meet credential.
5. Set the actual owner timezone, weekly hours, temporary and recurring blackouts, locations and their hours, meeting types, gaps, and reminders. Choose blocking calendars explicitly. Initial hours and gaps are proposals, not confirmed availability.
