...
`npm run check:offline` checks pinned shared-template synchronization, Worker and browser TypeScript, real Workers-runtime tests with mocked providers, the production Jekyll build, Chromium booking/admin/accessibility flows, and a Wrangler dry deployment. `npm run check` additionally captures synthetic rendered output and requires live Jev; see [Quality](QUALITY.md#required-jev-development-check). Browser fixtures never send real invitations. Browser screenshots are local under `work/frontend`; CI retains them briefly as artifacts. `npm run types` regenerates bindings after Wrangler changes. `npm run format` formats consumer code without modifying pinned dependencies.
...
The public booking and guest rescheduling pickers show seven dates with available slots per page, starting after minimum notice. Fully unavailable dates are skipped, including weekends, blackouts and fully booked days. Next shows the next seven available dates; the final page may contain fewer. Dates use the visitor's selected time zone, every slot on one date stays on that page, and the label names the first and last displayed dates. The configured booking horizon remains measured from the visit time, not from the first available date. If minimum notice reaches the horizon, the picker shows no availability without making a date-range request.

The shared picker scans consecutive ranges of seven local dates using the existing availability endpoint, stopping after finding one date beyond the page or reaching the horizon. That lookahead determines whether Next is enabled; each request stays within the eight-elapsed-day API maximum across daylight-saving transitions. Sparse schedules can require multiple reads, bounded by the configured horizon. It does not retain a second availability cache. Changing location refreshes from the current page's starting date, and changing time zone interprets that date in the new zone; returning to an earlier page rereads availability. Provider failures fail the entire page, including lookahead, rather than silently skipping dates. The usual single automatic retry, cancellation, stale-response protection and private rescheduling authorization apply to every scan.
...
**Settings → Your booking page → Display name** supplies the host name for new calendar invitations as well as the booking page and emails. New invitation titles use `Meeting type · Host name & Guest name`; the description identifies both participants in English or Spanish. The host attendee uses the connected Google account address shown under Calendar connections, with an accepted response; the guest's response is left for the guest. If both addresses match (ignoring case), only one attendee is sent. This uses the existing account connection and display setting, so forks need no additional identity configuration. A legacy whitespace-only display name falls back to the connected address in the invitation.
