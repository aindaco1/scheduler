...
Enable GitHub Actions and private vulnerability reporting on your fork, then update the reporting URLs in [the security policy](SECURITY.md) and its root entry point. Check runs on PRs, main pushes and a weekly schedule; Dependabot proposes dependency/action updates. Scheduled workflows in forks may need explicit activation. No deployment credential is stored in GitHub and no workflow deploys automatically.
