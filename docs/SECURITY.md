...
- Admin, management and API responses are private/no-store/no-transform with noindex, strict referrer policy and restrictive CSP. Only content-addressed public assets/logos have long cache lifetimes. Guest management has separate read/mutation rate limits; invalid reschedule targets are rejected before provider reads.
...
- Outbound requests do not automatically follow redirects, and deadlines cover response-body reads. JSON/iCloud parsing failures expose only bounded error classifications. Zoom refreshes cannot restore a removed or replaced credential.

The [1.0.1 engineering audit](release-evidence/security-audit-1.0.1-2026-09-10.md) records scope, seven resolved findings, synthetic reproductions, data inventory and exclusions. The [quality review](QUALITY.md) owns regression commands. Keep `npm run check` and `npm run audit:dependencies` green; the latter checks npm and locked Ruby dependencies. Do not weaken provider failure handling to improve apparent uptime.

Use the [recovery procedure](OPERATIONS.md#backup-and-recovery) before restoring storage. A restore can revive consumed tokens and queued messages; quarantine the restored state before reconnecting providers. The isolated drill verifies platform recovery, not possession of a separately backed-up production encryption key or an independent archival backup.
