...
Run `npm run check` before submitting a change. It also requires live Jev on fresh synthetic browser/email output; [Quality](QUALITY.md#required-jev-development-check) owns setup and interpretation. `npm run check:offline` explicitly excludes that semantic gate. It includes type checks, Worker tests, production build, asset/SEO/i18n/privacy gates, independent fork setup, browser accessibility/flow tests and a Wrangler dry run. `npm run audit:dependencies` checks current npm and Ruby advisories; inability to retrieve advisories is not a clean result. CI does this too.
